...
<section class="relative py-20 bg-gradient-to-br from-purple-900 to-indigo-900 overflow-hidden">
    ...
    <div class="relative max-w-7xl mx-auto px-4 text-center" data-aos="zoom-in">
        ...
        <div class="type-container mb-8 h-20">
            <span class="text-2xl md:text-3xl font-semibold text-cyan-300" id="typed-academic"></span>
        </div>
    ...
    <div class="max-w-7xl mx-auto px-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            @foreach([
                ...
                ['M. C. Judith Pérez Marcial', 'judith_perez.jpg', 'Investigadora', 'Derrolladora Web', ['fas fa-palette', 'fas fa-mobile-alt']],
                ...
                <div class="group relative bg-white/10 backdrop-blur-lg rounded-2xl p-6 shadow-2xl hover:shadow-purple-500/100 transition-all duration-300 hover:-translate-y-2" data-aos="fade-up">
                    <div class="relative w-48 h-48 mx-auto mb-6 overflow-hidden rounded-2xl shadow-xl">
                        <img src="{{ asset('images/miembros/' . $item[1]) }}" alt="{{ $item[0] }}" 
...
<section class="py-20 bg-gradient-to-br from-white to-indigo-50">
    <div class="max-w-7xl mx-auto px-4">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="relative bg-white rounded-2xl p-8 shadow-xl hover:shadow-2xl transition-shadow duration-300" data-aos="fade-left">
...
<!-- Sección de logros con animaciones numéricas -->
<section class="py-20 bg-purple-900 text-white">
    <div class="max-w-7xl mx-auto px-4">
        <div class="text-center mb-16">
            <h2 class="text-4xl font-bold mb-4 bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 to-purple-300">
                Impacto Cuantificable
            </h2>
            <p class="text-xl text-purple-200/90 max-w-2xl mx-auto">
                Nuestro trabajo se traduce en resultados concretos que mejoran vidas diariamente
            </p>
        </div>
        
        <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
            <div class="text-center p-6" data-aos="flip-left">
                <div class="text-5xl font-bold mb-4 text-cyan-400 countup" data-count="95">0</div>
                <div class="text-sm uppercase tracking-widest text-purple-300">Precisión Diagnóstica</div>
                <div class="h-1 bg-cyan-500/30 mt-4 mx-auto w-24"></div>
            </div>
            
            <div class="text-center p-6" data-aos="flip-left" data-aos-delay="200">
                <div class="text-5xl font-bold mb-4 text-purple-400 countup" data-count="30">0</div>
                <div class="text-sm uppercase tracking-widest text-purple-300">Países Alcanzados</div>
                <div class="h-1 bg-purple-500/30 mt-4 mx-auto w-24"></div>
            </div>
            
            <div class="text-center p-6" data-aos="flip-left" data-aos-delay="400">
                <div class="text-5xl font-bold mb-4 text-indigo-400 countup" data-count="150">0</div>
                <div class="text-sm uppercase tracking-widest text-purple-300">Proyectos Activos</div>
                <div class="h-1 bg-indigo-500/30 mt-4 mx-auto w-24"></div>
            </div>
            
            <div class="text-center p-6" data-aos="flip-left" data-aos-delay="600">
                <div class="text-5xl font-bold mb-4 text-cyan-300 countup" data-count="500">0</div>
                <div class="text-sm uppercase tracking-widest text-purple-300">Colaboradores</div>
                <div class="h-1 bg-cyan-500/30 mt-4 mx-auto w-24"></div>
            </div>
        </div>
    </div>
...
<script> // Animación de números 
    function startCounters() { const counters = document.querySelectorAll('.countup'); counters.forEach(counter => { const target = +counter.getAttribute('data-count'); const duration = 2000; const step = target / (duration / 50); let count = 0; const updateCount = () => { count += step; if (count < target) { counter.textContent = Math.ceil(count); setTimeout(updateCount, 50); } else { counter.textContent = target + '+'; } }; updateCount(); }); } // Iniciar contadores cuando la sección es visible 
    const observer = new IntersectionObserver((entries) => { entries.forEach(entry => { if (entry.isIntersecting) { startCounters(); observer.unobserve(entry.target); } }); }, { threshold: 0.5 }); document.querySelectorAll('.countup').forEach(counter => { observer.observe(counter.parentElement); }); 
    // Typing animation new Typed
</script>
<style> .countup { font-family: 'Space Grotesk', sans-serif; text-shadow: 0 2px 10px rgba(34, 211, 238, 0.3); } .group:hover .transform { transform: scale(1.05); } .animate-pulse { animation: pulse 3s cubic-bezier(0.4, 0, 0.6, 1) infinite; } @keyframes pulse { 0%, 100% { opacity: 1; } 50% { opacity: 0.5; } } .animate-spin-slow { animation: spin 8s linear infinite; } @keyframes spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } } </style>
@endsection
